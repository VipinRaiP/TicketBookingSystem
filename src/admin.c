...
/*
int updateNoUsers(int uid,int userLockFd,int newuid){
	...
	acc.totalUsers += 1;
	acc.uid[totalUsers-1] = newuid;

        lseek(fd,sizeof(struct account)*(uid-1),SEEK_SET);
        if(write(fd,&acc,sizeof(struct account)) != sizeof(struct account)){
                printf("write error\n");
                return 0;
        }
...
int updateNotrains(int uid,int userLockFd,int newtrainNo){

	struct train tr;


        lseek(userLockFd,sizeof(struct train)*(uid-1),SEEK_SET);
        if(read(userLockFd,&tr,sizeof(struct train)) != sizeof(struct train)){
                printf("write error\n");
                return 0;
        }

	acc.totalTrains += 1;
	acc.uid[totalTrains-1] = newtrainNo;

        lseek(userLockFd,sizeof(struct train)*(uid-1),SEEK_SET);
        if(write(userLockFd,&tr,sizeof(struct train)) != sizeof(struct train)){
                printf("read error\n");
                return 0;
        }
